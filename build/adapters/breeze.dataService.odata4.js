(function (definition, global) {
  ...
  proto.fetchMetadata = function (metadataStore, dataService) {
        ...
        schema.entityType && schema.entityType.forEach(function (et) {
          ...
          et.navigationProperty && et.navigationProperty
            ...
            .forEach(function (np) {
              ...
              getPossibleAssociations(et, np, schema)
                .forEach(function (a) {
                  if (association) return;
                  schema.association.forEach(function (sa) { if (sa.name === a.name) { association = sa; } });
                });

              // The association doesn't exist. Create it.
              if (!association) {
                association = getPossibleAssociations(et, np, schema)[0];
    ...
    function getPossibleAssociations(entityType, csdlProperty, schema) {
      // All associations are simply named as "[SourceRole]_[ParterRole]"
      ...
      if (csdlProperty.partner) {
        ...
        // TODO: only one possibility if the multiplicities are assymetrical.
        return [
          {
        ...
        return [{
          name: entityType.name + "_" + csdlProperty.name,
            ...
            { multiplicity: "1", role: entityType.name, type: schema.namespace + "." + entityType.name },
  ...
  proto.jsonResultsAdapter = new JsonResultsAdapter({
    ...
    visitNode: function (node, mappingContext, nodeContext) {
      ...
      if (metadata != null) {
        ...
        if (et && (et._mappedPropertiesCount <= Object.keys(node).length - 1 || !mappingContext.query)) {
      ...
      // BUG: Breeze ain't tracking our navigation properties when querying with '$expand'.
      if (!result.entityType && ["navProp", "navPropItem"].indexOf(nodeContext.nodeType) >= 0) {
        result.entityType = nodeContext.navigationProperty.entityType; // TODO: How does this work with inheritance?
      }
  ...
  function ODataV4ChangeRequestInterceptor(saveContext, saveBundle) {
    ...
    function getChangedNavigationProperties(entity) {
      ...
      return stype.getProperties()
        .filter(function (prop) { return prop.isNavigationProperty; })
        .filter(function (prop) {
